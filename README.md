...
<div id="controls-hint">Arrow keys / WASD to move &bull; SPACE / UP to jump (x2) &bull; stomp Dr Ivel 3x to defeat him</div>
...
var CHARS = [
  {name:'Jaime', color:'#7f77dd'},
  {name:'Vik',   color:'#1d9e75'},
  {name:'Adam',  color:'#d85a30'},
  {name:'John',  color:'#ba7517'},
  {name:'Vim',   color:'#d4537e'},
];
...
var W, H, selChar = 0, score = 0, lives = 3, gameRunning = false;
var player, platforms, drivels, drIvel;
var frameCount = 0, speed = 2, spawnTimer = 0, drIvelTimer = 0;
var keys = {};
...
document.querySelectorAll('.char-btn').forEach(function(b) {
  b.addEventListener('click', function() {
    document.querySelectorAll('.char-btn').forEach(function(x){ x.classList.remove('sel'); });
    b.classList.add('sel');
...
document.addEventListener('keydown', function(e) {
  keys[e.code] = true;
  if(['Space','ArrowUp','ArrowLeft','ArrowRight','ArrowDown','KeyW','KeyA','KeyD','KeyS'].indexOf(e.code) > -1) e.preventDefault();
});
...
function buildPlatforms() {
  ...
  return [
    ...
    {x:W/2-80,   y:g-240,   w:160, h:14, color:'#3c3289'},
...
function initGame() {
  ...
  var g = H - 60;
  player = {x:W/2-20, y:g-60, w:40, h:56, vx:0, vy:0, onGround:false, char:CHARS[selChar], invincible:0, jumpCount:0};
...
function spawnDrivel() {
  var word = DRIVEL_WORDS[Math.floor(Math.random()*DRIVEL_WORDS.length)];
  var fromLeft = Math.random() < 0.5;
  var y = Math.random()*(H*0.7) + H*0.05;
  var spd = speed + Math.random()*1.8;
  drivels.push({
    x: fromLeft ? -220 : W+220, y:y,
    vx: fromLeft ? spd : -spd,
    vy: (Math.random()-0.5)*0.9,
    word:word, size: word==='DRIVEL'?20:13,
...
function spawnDrIvel() {
  var fromLeft = Math.random() < 0.5;
  drIvel = {
    x: fromLeft ? -70 : W+70, y:H-130,
    vx: fromLeft ? speed*1.3+1.5 : -(speed*1.3+1.5),
    vy:-2, phase:0, health:3, hitFlash:0,
...
function update() {
  ...
  spawnTimer++;
  var rate = Math.max(25, 85 - frameCount/70);
  if(spawnTimer > rate) { spawnDrivel(); spawnTimer = 0; }

  for(var i=drivels.length-1; i>=0; i--) {
    var d = drivels[i];
    d.x += d.vx; d.y += d.vy; d.angle += 0.008;
    if(d.x < -350 || d.x > W+350 || d.y > H+150 || d.y < -200) { drivels.splice(i,1); continue; }
    if(player.invincible === 0) {
      var pr = {x:player.x+5, y:player.y+5, w:player.w-10, h:player.h-10};
      var dr = {x:d.x-50, y:d.y-16, w:100, h:32};
      if(overlap(pr,dr)) { drivels.splice(i,1); loseLife(); return; }
    }
  }
  ...
  if(drIvel) {
    ...
    for(var i=0; i<platforms.length; i++) {
      ...
      if(drIvel.x+50 > p.x && drIvel.x < p.x+p.w &&
        ...
        drIvel.y = p.y-70; drIvel.vy = 0;
        if(Math.random() < 0.015) drIvel.vy = -11;
      }
    }

    if(drIvel.y > H+150) { drIvel = null; return; }

    if(drIvel) {
      var pr = {x:player.x+5, y:player.y+5, w:player.w-10, h:player.h-10};
      var dr2 = {x:drIvel.x, y:drIvel.y, w:50, h:70};

      if(player.vy > 0 && pr.y+pr.h-player.vy <= drIvel.y+10 && overlap(pr,dr2)) {
        drIvel.health--;
        drIvel.hitFlash = 22;
      ...
      if(player.invincible === 0 && overlap(pr,dr2)) { loseLife(); return; }
    }
  }
  updateHUD();
}
...
function draw() {
  ...
  for(var i=0; i<70; i++) {
    var sx = (i*139 + frameCount*0.04) % W;
    var sy = (i*101 + frameCount*0.015) % H;
  ...
  for(var i=0; i<drivels.length; i++) {
    var d = drivels[i];
    ctx.save();
    ctx.translate(d.x, d.y);
    ctx.rotate(d.angle);
    var col = d.isBig ? '#e24b4a' : '#7f77dd';
  ...
  if(drIvel) {
    ...
    var bob = Math.sin(drIvel.phase*0.09)*3;

    ctx.fillStyle = '#e8e8f4';
    rr(-18, 22+bob, 36, 48, 5); ctx.fill();
    ...
    for(var h=0; h<drIvel.health; h++) drawStar(-16+h*16, -18+bob, 7, '#e24b4a');
    ctx.fillStyle='#a32d2d'; ctx.font='bold 9px monospace'; ctx.textAlign='center';
    ctx.fillText('DR IVEL', 0, 78+bob);

    if(drIvel.bubbleTimer > 0) {
      var bw = Math.min(drIvel.bubble.length*6+18, 170);
      ctx.fillStyle='rgba(255,255,255,0.93)';
      rr(-bw/2, -58+bob, bw, 26, 6); ctx.fill();
      ctx.fillStyle='#a32d2d'; ctx.font='bold 9px monospace'; ctx.textAlign='center';
  ...
  if(player) {
    ...
    var bob = player.onGround ? Math.sin(frameCount*0.22)*2 : 0;

    ctx.fillStyle = player.char.color;
    rr(-15, 22+bob, 30, 34, 6); ctx.fill();
